...
northsea <- make.region(region.name = "minkes",
                      shape = "C:\\Users\\erexs\\Documents\\simulation-strata\\shapefiles\\Strataprj.shp",
...
minkes <- read_sf("C:\\Users\\erexs\\Documents\\simulation-strata\\shapefiles\\Strataprj.shp")
...
When employing model selection to choose between models with pooled detection functions and stratum-specific detection functions, the most worrisome cases of bias arising when $\Delta << 1$ are eliminated.  Bias in stratum-specific density estimates become more pronounced as $\Delta >> 1$.  This is likely caused by poor model selection performance (shown in preceding table) for large values of $\Delta$.  When $\Delta$ is at its maximum, the probability of using the model that produces unbiased stratum-specific estimates is `r round(pooled[nrow(pooled), 10],2)`
...
Confidence interval coverage approaches nominal levels when $\Delta \approx 1$.
...
There are two take away messages from this model selection assessment.  In only two instances when the stratum-specific scale parameters differ is there a greater than 50:50 chance that the correct (stratum-specific detection functions) model is chosen.  This is under the scenario when the survey design is superb: adequate number of replicate transects resulting in large numbers (>60) of detections in each stratum.  With a <50% chance of selecting the correct model for inference, the odds favour inferring there is no difference between strata in detection functions, employing the pooled detection function as the basis of inference.  This model produces biased stratum-specific density estimates.  Hence, do not rely upon model selection tools to mitigate the risk of reporting biased estimates.
...
```{r hissgood, fig.cap="Distribution of deltaAIC with 40 transects in the study area.", layout="l-body-outset", fig.width=6, fig.height=6.5}
par(mfrow=c(4,2))
for (i in seq_along(delta.multiplier)) {
...
```{r hissbad, fig.cap="Distribution of deltaAIC with 25 transects in the study area.",layout="l-body-outset", fig.width=6, fig.height=6.5}
par(mfrow=c(4,2))
for (i in seq_along(delta.multiplier)) {
...
Buckland, S.T., Rexstad, E., Marques, T.A. and Oedekoven, C.S. 2015. Distance Sampling: Methods and Applications. Springer.